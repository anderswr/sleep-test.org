...
export default function SleepTimeline({ bedtime, waketime, onChange }: Props) {
  const [mode24, setMode24] = React.useState(true);
  const [start, setStart] = React.useState(timeToIdx(bedtime) || 22); // default ~22:00
  const [end, setEnd] = React.useState(timeToIdx(waketime) || 14);    // default ~07:00 (14 * 30min = 07:00)
  const draggingRef = React.useRef<null | "start" | "end">(null);
  const trackRef = React.useRef<HTMLDivElement | null>(null);

  React.useEffect(() => {
    if (bedtime) setStart(timeToIdx(bedtime));
    if (waketime) setEnd(timeToIdx(waketime));
  }, [bedtime, waketime]);

  // Begrens til 0..47
  function clamp(v: number) { return Math.max(0, Math.min(47, v)); }
  ...
  function onPointerMove(e: PointerEvent) {
    ...
    const idx = clamp(Math.round(ratio * 47));
  ...
  const startPct = (start / 47) * 100;
  const endPct = (end / 47) * 100;
    ...
    <div className="sleep-timeline">
      <div className="sleep-head">
        ...
        <div className="sleep-mode">
          <button
            type="button"
            className={`btn tiny ${mode24 ? "active" : ""}`}
            onClick={() => setMode24(true)}
            aria-pressed={mode24}
            title="24h"
          >24h</button>
          <button
      ...
      <div className="sleep-track" ref={trackRef}>
        {/* segment */}
        <div
          className={`sleep-segment ${end < start ? "wrap" : ""}`}
          style={segmentStyles()}
          aria-hidden
        />
        {/* ticks */}
