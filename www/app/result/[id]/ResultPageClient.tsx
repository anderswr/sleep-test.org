...
export default function ResultPageClient({ id }: ResultPageClientProps) {
  ...
  const shareMessage = t(
    dict,
    data
      ? "ui.result.share_message_with_score"
      : "ui.result.share_message",
    data
      ? "I took a free sleep test and scored {score}/100. Try your own anonymous test:"
      : "I took a free sleep test. Try your own anonymous test:"
  );

  const shareText = data
    ? shareMessage.replace("{score}", String(Math.round(data.sleepScore)))
    : shareMessage;

  const shareTargets = buildShareTargets(shareUrl, shareText);

  const shareButtons = [
    { key: "facebook", label: t(dict, "ui.result.share_targets.facebook", "Facebook"), href: shareTargets.facebook },
    { key: "x", label: t(dict, "ui.result.share_targets.x", "X"), href: shareTargets.x },
    { key: "linkedin", label: t(dict, "ui.result.share_targets.linkedin", "LinkedIn"), href: shareTargets.linkedin },
    { key: "reddit", label: t(dict, "ui.result.share_targets.reddit", "Reddit"), href: shareTargets.reddit },
    { key: "whatsapp", label: t(dict, "ui.result.share_targets.whatsapp", "WhatsApp"), href: shareTargets.whatsapp },
    { key: "telegram", label: t(dict, "ui.result.share_targets.telegram", "Telegram"), href: shareTargets.telegram },
    { key: "email", label: t(dict, "ui.result.share_targets.email", "Email"), href: shareTargets.email },
  ];
                ...
                <div>
                  ...
                  <div className="share-chip-row">
                    <button className="share-chip" onClick={() => shareOrCopy("instagram")}>
                      ...
                      <span>{t(dict, "ui.result.share_instagram", "Instagram")}</span>
                    </button>
                    <button className="share-chip" onClick={() => shareOrCopy("tiktok")}>
                      <span aria-hidden>
                        <IconTiktok />
                      </span>
                      <span>{t(dict, "ui.result.share_tiktok", "TikTok")}</span>
                    </button>
